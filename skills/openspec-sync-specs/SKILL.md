...
4. **For each delta spec, apply changes to main specs**
   ...
   d. **Create new main spec** if capability doesn't exist yet:
      ...
      - Add Purpose section: copy the delta's `## Purpose` body verbatim when it has one
        (this is what `openspec archive` does); otherwise write a concise Purpose supported by the accepted requirements
...
6. **Show summary**
   ...
   - Any missing source rationale that needs resolution before synchronization
