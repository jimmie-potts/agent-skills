...
| Simple bounded item, both hosts | Section opens with `**Start with:**` naming `One-shot` first; table has Model, Thinking level, Session type, Subagents and Availability; both prompt blocks follow. |
| High-impact one-line fix | `One-shot` at the stronger floor with a Checkpoints row when decisions need a stop; prompts state Opus/high and Sol/high as the user's selection. |
