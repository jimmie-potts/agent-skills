...
| Reviewer spawned with an `opus` request; it returns findings without naming its model | Entry reads `requested opus/default, model unknown (unknown), level unknown (unknown)`; the request never becomes `opus (host-observed)`. |
