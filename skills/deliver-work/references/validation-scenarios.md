...
| Mechanical documentation change; impact low, uncertainty low, checks strong, supported model overrides available | Select capable lower-cost reviewers for both axes: Opus on Claude at the session's inherited level, Luna/high on Codex with its provisional evidence disclosed. Do not inherit the coordinator model by omission. This is a simulated selection, not measured reviewer quality. |
...
| Pasted prompt states Sonnet at medium, one-shot with two Sonnet reviewers; runtime instructions name Opus | State the running model and stop before delivery; do not ask the agent to report its effort. |
...
| Pasted one-shot prompt authorizes two Sonnet reviewers | Implement without worker subagents; run the two reviewers in fresh read-only contexts without asking again. |
...
| Item's Execution recommendation starts One-shot on Sonnet at medium; pasted prompt states Sonnet at medium; runtime instructions name Sonnet; `CLAUDE_EFFORT` is `medium` | Copy `One-shot, sonnet, medium` into `Recommended`; record the level as `medium (user-stated) + medium (host-observed)` without asking the agent for its effort. |
| Reviewer spawned with an `opus` request; it returns findings without naming its model | Entry reads `requested opus at default, model unknown (unknown), level unknown (unknown)`; the request never becomes `opus (host-observed)`. |
| Resumed delivery lost earlier review contexts and finding counts | `Agents` gives a known minimum, counts stay `unknown` or `at least <n>`, and no cell reads zero from missing history. |
| Item has only a prose planned-start line and no `## Execution recommendation` section | `Recommended` is `none`; do not retype the prose into the row. |
| Change repairs a merged delivery; the coordinator's session index exposes a raw session ID | Add `**Fixes delivery:**` naming the merged PR; omit `Session label` or use a neutral label, never the raw ID. |
| Delivery runs without a durable record because PR writes are not authorized | Include the complete section in the final response; create no PR or comment to hold it. |
