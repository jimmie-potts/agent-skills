...
Read the consuming repository's agent instructions and planning configuration.
Run every CLI example below through its pinned local command when one is
provided; bare `openspec` is shorthand for that command. Resolve the intended
planning root and change from the request before acting. This skill grants no
new filesystem, tracker, publication, or implementation authority. A planning-
...
**Planning boundary**: This substep prepares planning artifacts within the underlying request's authority. Do not edit product code inside it. A direct planning-only request stops at its requested boundary. When the user already authorized implementation, present the plan, settle required decisions, and return to the delivery coordinator for apply without another permission round.
...
Continue to apply only when the underlying request authorizes implementation and the required decisions are settled.
...
- State the next step within the requested terminal state. Return to an authorized delivery coordinator when one composed this proposal.
