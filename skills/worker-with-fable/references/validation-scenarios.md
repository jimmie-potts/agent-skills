...
| Tier selection | The composing workflow supplies a bounded low-rated task. The coordinator selects `sonnet` by default and `opus` when the assessment shows high complexity or impact, or a completed default Sonnet attempt has failed acceptance and reassessment selects a stronger tier. A `fable` worker is refused as a non-pairing, and a `haiku` worker is refused as unsupported. |
