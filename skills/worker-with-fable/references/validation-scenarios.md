...
| Host routing | A Codex host with collaboration tools does not select this skill; the coordinator reports that sol-with-astra applies. A Claude Code host does not select sol-with-astra. Selection follows verified tool schemas, not the names in the request. |
